...
mod tests {
    ...
    fn assert_sapl_eq(left: &str, right: &str) {
        assert_eq!(
            crate::parse_sapl(left.as_bytes()),
    ...
    fn literal_test() {
        ...
        assert_toks_eq(&tokens, vec![Tokens::TString("Hello World".to_owned())]);
    }
    ...
    fn parenthesis_eval() {
        ...
        assert_val_eq(
            "'Hello ' + (4000 + 410 * 2)",
            Values::Str("Hello 4820".to_owned()),
        );
    }
    ...
    fn conditional_test() {
        ...
        assert_parse_str_eq(
            "if true: false else if true: true",
            Ast::If(
                Box::new(Ast::VBool(true)),
                Box::new(Ast::VBool(false)),
                Some(Box::new(Ast::If(
                    Box::new(Ast::VBool(true)),
                    Box::new(Ast::VBool(true)),
                    None,
    ...
    fn func_test() {
        ...
        assert_parse_str_eq(
            ...
            Ast::Bop(
                ...
                Op::Mult,
                Box::new(Ast::FnApply(
                    Box::new(Ast::Name("func".to_owned())),
                    vec![Box::new(Ast::VInt(3))],
                )),
            ),
        );
    ...
    fn higher_order_function_test() {
        ...
        assert_toks_eq(
            ...
            vec![
                Tokens::Let,
                Tokens::Name("add10".to_owned()),
                Tokens::OpAssign,
                Tokens::Name("add".to_owned()),
                Tokens::LParen,
                Tokens::Integer(10),
                Tokens::Comma,
                Tokens::OpQ,
        ...
        assert_parse_str_eq(
            "let add10 = add(10, ?)",
            Ast::Let(
                vec![("add10".to_owned(), false)],
                Box::new(Ast::FnApply(
                    Box::new(Ast::Name("add".to_owned())),
        ...
        assert_parse_str_eq(
            ...
            Ast::Bop(
                Box::new(Ast::Bop(
                    ...
                    Op::Pipeline,
                    Box::new(Ast::FnApply(
                        Box::new(Ast::Name("sub".to_owned())),
                        vec![Box::new(Ast::Placeholder), Box::new(Ast::VInt(3))],
                    )),
                )),
                Op::Pipeline,
    ...
    fn tuple_test() {
        ...
        assert_val_eq(
            "let tup = (10, 'a', 'c'); let a, b, c = tup; a + b + c",
            Values::Str("10ac".to_owned()),
        );
        assert_val_eq("let min, max = -1..100; min - max", Values::Int(-101));
        assert_val_eq(
            "let _, _, name = (42, 'Corsair', 'Jim'); name",
            Values::Str("Jim".to_owned()),
        );
    }
    ...
    #[test]
    fn map_test() {
        let mut m = HashMap::<Values, Values>::new();
        m.insert(
            Values::Str("name".to_owned()),
            Values::Str("Billy".to_owned()),
        );
        m.insert(Values::Str("age".to_owned()), Values::Int(53));
    ...
    fn return_test() {
        assert_parse_str_eq(
            ...
            Ast::If(
                ...
                Box::new(Ast::Uop(Box::new(Ast::Name("x".to_owned())), Op::Return)),
                None,
            ),
        );
    ...
    fn ref_tests() {
        ...
        *x + 20
        ...
        assert_eval_eq(
            r#"let var n = 1; let p = &&n; p = 3"#,
            Res::Exn(Values::Str(
                "Cannot mutate an immutable value: p".to_owned(),
            )),
        );

        assert_sapl_eq(
            r#"try { let var n = 1; let p = &n; p <- 3 } catch _: true"#,
            "true",
        );
    ...
    #[test]
    fn struct_tests() {
        let st = crate::parser::SaplStruct {
            name: "Person".to_owned(),
            ctor: Some(Box::new(Ast::Func(
                "Person".to_owned(),
                vec![("name".to_owned(), false, None)],
                Box::new(Ast::VInt(0)),
                None,
            ))),
            dtor: Some(Box::new(Ast::Func(
                "`Person".to_owned(),
                Vec::<(String, bool, Option<Box<Ast>>)>::new(),
                Box::new(Ast::VInt(0)),
                None,
            ))),
            publics: vec![
                ("name".to_owned(), false, None),
                ("age".to_owned(), true, Some(Box::new(Ast::VInt(0)))),
                (
                    "hello".to_owned(),
                    false,
                    Some(Box::new(Ast::Func(
                        "hello".to_owned(),
                        Vec::<(String, bool, Option<Box<Ast>>)>::new(),
                        Box::new(Ast::VInt(0)),
                        None,
    ...
    fn include_test() {
        ...
        assert_parse_str_eq(
            ...
            Ast::Import(
                "examples/im_in_test.sapl".to_owned(),
                Some("test".to_owned()),
            ),
        );
    ...
    fn string_test() {
        ...
        assert_eval_eq(
            ...
            Vl(Values::Str("\x1b[31mTest\x1b[0m".to_owned())),
        );
    }
